...
# Configuración de Matplotlib para modo Headless
import matplotlib
matplotlib.use('Agg')
...
class WeatherTraderV6_5_2:
    ...
    def ejecutar_trade(self, ciudad, config):
        ...
        try:
            ...
            # ==== Resumen de edges potenciales ====
            print(f"\n📌 {ciudad} | Forecast Temp: {t_forecast:.1f}°C | Sigma: {sigma:.2f}")
            print(f"{'Pregunta':60} {'YES':>6} {'NO':>6} {'Prob':>6} {'Edge':>6}")
            ...
            for mkt in res_gamma:
                ...
                print(f"{pregunta[:60]:60} {p_yes:6.3f} {p_no:6.3f} {prob:6.3f} {edge:6.3f}")
                ...
                if edge>edge_threshold:
                    ...
                    if kelly_f>0.01:
                        ...
                        riesgo_total+=stake
                        print(f"🎯 TRADE ABIERTO: {ciudad} | {lado} | Edge: {edge:.2%}")
    ...
    def generar_reporte_grafico(self):
        try:
            ...
            print(f"📊 Reporte analítico generado en /reports/")
        except Exception as e: print(f"⚠️ Error en analítica: {e}")
